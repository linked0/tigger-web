...
export const HoverCard = styled(Card)`
  ...
  :hover {
    border: 1px solid ${({ theme }) => darken(0.06, theme.purpleBoxline2)};
  }
`;
...
export default function FullPositionCard({ pair, border }: PositionCardProps) {
  ...
  function sortsBOA(token: Token): boolean {
    return token.symbol !== "WDEV";
  }
  const currency0 = unwrappedToken(!sortsBOA(pair.token0) ? pair.token0 : pair.token1);
  const currency1 = unwrappedToken(sortsBOA(pair.token0) ? pair.token0 : pair.token1);
  const [showMore, setShowMore] = useState(false);
